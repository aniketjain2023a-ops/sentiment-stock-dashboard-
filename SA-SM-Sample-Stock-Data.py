...
def analyze_sentiment(articles, stock_prices):
    ...
    for article in articles:
        ...
        article_date = pd.to_datetime(article['publishedAt']).date()

        article_date = pd.to_datetime(article_date)
        
        closest_price = stock_prices.loc[stock_prices.index <= article_date].iloc[-1]['closing_price'] if not stock_prices.loc[stock_prices.index <= article_date].empty else np.nan
...
stock_prices = pd.DataFrame({
    ...
    'closing_price': [150, 155, 160, 158, 162]
})
