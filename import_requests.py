...
api_key = 'Yj4jv16LOdc6KzrfnZChneQnSsSQCRP51h3A5h0H' 
# api_key = 'd4a40858d71f4397a7de8ec8fa267041'
ALPHA_VANTAGE_API_KEY = 'EN726KB6B9ZTPXVN'
company = 'apple'
...
def analyze_sentiment(articles, stock_prices):
    ...
    for article in articles:
        # Skip if no title or publishedAt
        ...
        closest_price = (
            stock_prices.loc[stock_prices["date"] <= article_date].iloc[-1]["closing_price"]
            if not stock_prices.loc[stock_prices["date"] <= article_date].empty
            else np.nan
        )
    ...
    # Always return a DataFrame with these columns
    df = pd.DataFrame(sentiment_data, columns=["title","sentiment","date","source","closing_price"])
    
    if not df.empty:
        df["rolling_sentiment"] = df["sentiment"].rolling(window=7).mean()
        df["rolling_closing_price"] = df["closing_price"].rolling(window=7).mean()
    else:
        print("⚠️ Warning: No articles found from NewsAPI. DataFrame is empty.")
